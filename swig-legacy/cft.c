/*
 *
 *   FILE cft.c
 *
 */

#include <math.h>

#define SWAP(a,b) do { double temp=(a);(a)=(b);(b)=temp; } while( 0 )

void cosfft1(double *data, unsigned n, int isign)
{
    int     i, j;
    unsigned  m;
    int     mmax, istep, i1, i2, i3, i4;
    double  wr, wi, wpr, wpi, wtemp, theta,
            y1, y2, tempr, tempi, h1r, h1i, h2r, h2i,
            temp, sum, first, last;

    temp=data[n];
    if (isign == -1) {  /* inverse transform */
        first=0.5*data[0];
        last=0.5*temp;
    }
    sum=0.0;
    for (j=0;j<=n;j+=2) sum += data[j];
    for (j=1;j<=n;j+=2) sum -= data[j];
    data[n]=sum;

    wtemp=sin(0.5*(theta=M_PI/n));
    wpr = -2.0*wtemp*wtemp;
    wpi=sin(theta); m=n >> 1;
    wr=1.0; wi=0.0;
    sum=data[0];
    for (j=1;j<=m;j++) {
        wr=(wtemp=wr)*wpr-wi*wpi+wr;
        wi=wi*wpr+wtemp*wpi+wi;
        y1=0.5*(data[j]+data[n-j]);
        y2=data[j]-data[n-j];
        data[j]=y1-wi*y2;
        data[n-j]=y1+wi*y2;
        sum += wr*y2;
    }
    j=1;
    for (i=1;i<=n;i+=2) {
        if (j > i) {
            SWAP(data[j-1],data[i-1]);
            SWAP(data[j],data[i]);
        }
        m=n >> 1;
        while (m >= 2 && j > m) {
            j -= m;
            m >>= 1;
        }
        j += m;
    }
    mmax=2;
    while (n > mmax) {
        istep=mmax << 1;
        theta=2.0*M_PI/mmax;
        wtemp=sin(0.5*theta);
        wpr = -2.0*wtemp*wtemp;
        wpi=sin(theta);
        wr=1.0; wi=0.0;
        for (m=1;m<=mmax;m+=2) {
            for (i=m;i<=n;i+=istep) {
                j=i+mmax;
                tempr=wr*data[j-1]-wi*data[j];
                tempi=wr*data[j]+wi*data[j-1];
                data[j-1]=data[i-1]-tempr;
                data[j]=data[i]-tempi;
                data[i-1] += tempr;
                data[i] += tempi;
            }
            wr=(wtemp=wr)*wpr-wi*wpi+wr;
            wi=wi*wpr+wtemp*wpi+wi;
        }
        mmax=istep;
    }
    theta=M_PI/(n >> 1);
    wtemp=sin(0.5*theta);
    wpr = -2.0*wtemp*wtemp; wi=wpi=sin(theta); wr=wpr+1;

    data[0] += data[1];
    m=n >> 2;

    for (i=1;i<=m;i++) {
        i2=(i1=i << 1)+1; i4=(i3=n-i2+1)+1;
        h1r=0.5*(data[i1]+data[i3]);
        h1i=0.5*(data[i2]-data[i4]);
        h2r=0.5*(data[i2]+data[i4]);
        h2i=0.5*(data[i3]-data[i1]);
        data[i1]=h1r+wr*h2r-wi*h2i;
        data[i2]=h1i+wr*h2i+wi*h2r;
        data[i3]=h1r-wr*h2r+wi*h2i;
        data[i4] = -h1i+wr*h2i+wi*h2r;
        wr=(wtemp=wr)*wpr-wi*wpi+wr;
        wi=wi*wpr+wtemp*wpi+wi;
    }
    data[1]=sum;
    for (j=3;j<=n;j+=2) {
        sum += data[j];
        data[j]=sum;
    }

    for (j=0;j<n;j+=2) {
        data[j    ] += temp;
        data[j + 1] -= temp;
    }

    if (isign == -1) {    /* inverse transform */
        for (j=0;j<n;j+=2) {
            data[j    ]=(data[j    ]-(first+last))*2.0/n;
            data[j + 1]=(data[j + 1]-(first-last))*2.0/n;
        }
        data[n]=(data[n]-(first+last))*2.0/n;
        data[0] *= 0.5;
        data[n] *= 0.5;
    }
}

void cft2(double **data, unsigned nn,int isign)
{
    int i, j, idim;

    for (idim=0;idim<2;idim++) {
        for (i=0;i<=nn;i++)
            cosfft1(data[i],nn,isign);
        for (i=0;i<=nn;i++)
            for (j=i;j<=nn;j++)
                SWAP(data[i][j],data[j][i]);
    }
}

#undef SWAP
